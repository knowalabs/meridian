...
How `@sonalsithara/devpilot` stores secrets, why AI-suggested file paths are never trusted directly, and which code paths in this repo demand extra care. Read this before touching `src/core/vault.ts`, `src/generate/artifacts.ts`'s `isAllowedPath`, or the harness config DevPilot generates for _other_ projects. Not covered: security advice for a target project being scanned — that's whatever `devpilot generate` writes into _that_ project's own `docs/security.md`, not this one.
...
| `FileVault`       | fallback (any platform, or forced via `DEVPILOT_VAULT=file`) | AES-256-GCM encrypted blob at `<devpilotHome>/keys/vault.enc`; the 256-bit master key lives separately at `<devpilotHome>/keys/.master`, written with `mode: 0o600`.              |

Neither `security` nor `secret-tool` can reliably enumerate stored items, so a non-secret index of account _names only_ (never values) is kept at `<devpilotHome>/keys/index.json`, written via `indexAdd`/`indexRemove`/`indexWrite` — never add secret material to this file.
...
`repairVault()` (`src/core/vault.ts`) backs up (`backupFile`) and removes `vault.enc`, `.master` and `index.json` so a corrupted vault can be reinitialized via `devpilot keys repair` — it never silently discards without a backup first.
...
## Generated harness safety (what DevPilot writes for other projects)
