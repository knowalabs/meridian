<!-- Generated by devpilot generate — edit .devpilot/rules.md and re-run to update. -->

# @sonalsithara/devpilot — Instructions for GitHub Copilot

## General — how to approach changes in this codebase

- This is `@sonalsithara/devpilot`, a Node.js/TypeScript CLI (`devpilot`) that makes other codebases AI-assistant-ready. Keep "the target project" (what `devpilot generate` scans, e.g. `src/scan/analyzer.ts`'s subject) strictly separate from "this repo" (DevPilot's own source) — never conflate the two.
...
- Prefer using the packaged skills for structural work in this repo: `add-artifact-kind` for a new generated artifact kind, `add-ai-provider` for a new provider, `debug-generate` for pipeline failures, `review-diff` before opening a PR, and `verify`/`release-devpilot` for the verification and release flows below.

## Architecture — module boundaries

- `src/index.ts` — process entry point only: Node-version gate (rejects `< 18`), global `uncaughtException`/`unhandledRejection`/`SIGINT` handlers, dispatch to `src/launcher.ts` (bare `devpilot` in a TTY) or `src/cli.ts` (`buildCli().parseAsync`). No command logic here.
...
- `src/generate/` — the AI-kit pipeline: `digest.ts` (`buildDigest`, `digestBudgetFor`), `artifacts.ts` (`ARTIFACT_KINDS`, `isAllowedPath`, `parseFileBlocks`, `FORMAT_SPEC`, `commonPrompt`), `pipeline.ts` (`runGenerate`, `generateKind`, `pickProvider`, `estimateGenerate`, `concurrencyFor`), `manifest.ts` (`KitManifest`, `fingerprintOf`, `diffFingerprints`, `fileStates` — powers `devpilot sync`), `cache.ts` (codebase-review caching keyed by project/provider/model/digest). Any AI-suggested file path must be validated through `isAllowedPath` before writing — never bypass it.
- `src/rules/generators.ts` — mirrors `.devpilot/rules.md` into every AI tool's native config file (`CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, etc.) via `generateRules`/`RULE_TARGETS`. Changes to rules content flow from here, not the other way around.
...
- Never write generated output (`.devpilot/`, `CLAUDE.md`, `AGENTS.md`, etc.) from anywhere except the `generate` pipeline and `src/rules/generators.ts` — every other module stays read-only with respect to the target project's files. This applies to DevPilot's own root-level generated files too (see Safety).
...
- Throw `CliError` (`src/core/errors.ts`) for all user-facing failures, never a raw `Error`. Always attach an actionable `hint` — see `classifyStatus` in `src/providers/router.ts` (`hint: 'Run "devpilot auth ${ctx.provider}"...'`) and `FileVault.readAll`'s catch block in `src/core/vault.ts`.
...
- Sandbox filesystem/vault state via `process.env.DEVPILOT_HOME` (temp dir) and `process.env.DEVPILOT_VAULT = 'file'`, cleaned up in `afterEach` with `fs.rmSync(..., { recursive: true, force: true })` — see `tests/doctor.test.ts` and `tests/commands.test.ts`. Never touch the real OS keychain or the developer's real `~/.devpilot`.
...
- Use the `devpilot-test-runner` agent to run the suite and diagnose failures without breaking coverage thresholds or test isolation, and the `debug-generate` skill for a failing `generate` run specifically.
...
- Never commit or write real API keys, tokens, or vault contents to disk outside `src/core/vault.ts`'s backends. `keys/index.json` under `devpilotHome()` (`indexWrite` in `src/core/vault.ts`) stores only account names, never secret values — do not add secret material to it.
...
- Treat `.devpilot/` and root-level generated files (`CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, `README_AI.md`) in _this_ repo's own working tree as dogfooded `devpilot generate` output, not hand-authored source — do not hand-edit generated sections; change `src/rules/generators.ts` or `src/generate/artifacts.ts` instead.
...
- Never run `npm publish`, push git tags matching `v*`, or modify `NPM_TOKEN`/CI secrets from an assistant session. Use the `release-devpilot`/`release` skill to prepare (not execute) a release.
...
- Turn `devpilot login` from a stub into a tracked feature boundary. `loginCommand` in `src/commands/update.ts` and the README both mark Cloud Sync as "on the roadmap, not available yet" — a registered CLI command with no working implementation. First step: when Cloud Sync design starts, gate any new authenticated-account state behind the same `Vault`/`core/config.ts` boundaries used for provider keys today, so local-only assumptions in `src/core/paths.ts`/`devpilotHome()` don't have to be unwound later.
