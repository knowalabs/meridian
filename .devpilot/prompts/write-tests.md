# Write Tests in DevPilot's Vitest Style
...
- Filesystem/vault sandboxing: set `process.env.DEVPILOT_HOME` to a fresh `fs.mkdtempSync(path.join(os.tmpdir(), 'devpilot-<area>-'))` and `process.env.DEVPILOT_VAULT = 'file'` in `beforeEach`; clean up with `fs.rmSync(dir, { recursive: true, force: true })` in `afterEach`. Never touch the real OS keychain or the developer's real `~/.devpilot`.
...
2. Set up the sandbox in `beforeEach`/`afterEach` exactly as described above — temp `DEVPILOT_HOME`, `DEVPILOT_VAULT=file`, cleanup — before writing any assertion.
