# Architecture — @sonalsithara/devpilot

## What this project is

`@sonalsithara/devpilot` is a Node.js/TypeScript CLI, invoked as `devpilot`, that makes _other_ codebases ("the target project") AI-assistant-ready in one command. It is distributed as a global npm package (`bin: { "devpilot": "./dist/index.js" }`) and does four jobs:

1. **Tool management** — detect/install/configure AI coding assistants (Claude Code, Codex CLI, Gemini CLI, Cursor, Copilot) via `devpilot doctor` / `install` / `uninstall`.
2. **Secret management** — store provider API keys in the OS-native secret store via `devpilot auth` / `keys`.
3. **AI-kit generation** — the flagship feature, `devpilot generate`: scan a target codebase, build a digest, have an AI provider (or a static fallback) write a codebase review, then generate rules/subagents/skills/commands/prompts/docs, mirrored into every AI tool's own config format.
4. **AI routing** — `devpilot ask` / `router` picks the best-suited provider by cost/speed/quality/context size, supporting both hosted HTTP APIs and keyless local CLIs (Claude Code, Codex CLI, Gemini CLI, Ollama).
