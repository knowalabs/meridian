...
describe('project analyzer', () => {
  ...
  beforeEach(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'meridian-scan-'));
    fs.writeFileSync(
  ...
  it('derives runnable commands for non-npm ecosystems, Makefile first', () => {
    const rust = fs.mkdtempSync(path.join(os.tmpdir(), 'meridian-rust-'));
    try {
  ...
  it('derives Python commands from pyproject tool mentions', () => {
    const py = fs.mkdtempSync(path.join(os.tmpdir(), 'meridian-py-'));
    try {
      fs.writeFileSync(
  ...
  it('detects Elixir, SwiftPM, Deno and .NET projects with their commands', () => {
    ...
    for (const [file, content, framework, scripts] of cases) {
      const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'meridian-eco-'));
      try {
  ...
  it('detects Django and Rails on top of their base ecosystems', () => {
    const dj = fs.mkdtempSync(path.join(os.tmpdir(), 'meridian-dj-'));
    const rails = fs.mkdtempSync(path.join(os.tmpdir(), 'meridian-rails-'));
    try {
  ...
  it('detects CI systems beyond GitHub Actions as conventions', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'meridian-ci-'));
    try {
  ...
  it('extracts symbols from non-JS languages', () => {
    ...
    for (const [file, content, expected] of cases) {
      const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'meridian-sym-'));
      try {
        fs.writeFileSync(path.join(dir, file), content);
        const a = analyzeProject(dir);
