...
function makeProject(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'devpilot-validate-'));
  fs.writeFileSync(
...
describe('validateArtifacts', () => {
  ...
  it('says nothing about scripts when the project declares none', () => {
    const bare = fs.mkdtempSync(path.join(os.tmpdir(), 'devpilot-noscripts-'));
    try {
      const files = agent(`${header}\nRun \`npm run build\`.\n`);
