...
describe('provider network behavior', () => {
  ...
  beforeEach(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'meridian-router-net-'));
  ...
  it('maps 401 to a CliError hinting at meridian auth', async () => {
    ...
    expect((err as CliError).hint).toContain('meridian auth anthropic');
  });
...
describe('claude-code CLI provider', () => {
  ...
  beforeEach(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'meridian-cc-'));
    process.env.MERIDIAN_HOME = tmp;
  });
  afterEach(() => {
    setRunForTests(null);
...
describe('codex-cli and gemini-cli providers', () => {
  ...
  beforeEach(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'meridian-cli-prov-'));
    process.env.MERIDIAN_HOME = tmp;
  });
  afterEach(() => {
    setRunForTests(null);
