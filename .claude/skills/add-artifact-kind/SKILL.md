...
description: Use when adding a new generated artifact kind (like rules, agents, skills, commands, prompts, docs, harness) to devpilot generate's ARTIFACT_KINDS.
---
...
Adding a brand-new kind of file `devpilot generate` produces (today: `rules`, `agents`, `skills`, `commands`, `prompts`, `docs`, `harness`, and the opt-in `ci`). Not for changing what an _existing_ kind writes — that's a direct edit to its `prompt`/`fallback` — and not for provider wiring, which is `@.claude/skills/add-ai-provider/SKILL.md`.
...
5. Update the `Kinds:` and `Opt-in kinds:` help text in the `generate` command's `addHelpText('after', …)` block in `src/cli.ts` so `devpilot generate --help` stays accurate.
...
- [ ] `devpilot generate --help`'s kind list is updated.
