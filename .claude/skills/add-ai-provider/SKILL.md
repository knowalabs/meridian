...
4. If the model has a published per-million-token price, add it to `MODEL_PRICING` in `router.ts` — otherwise leave it out; `pricingFor`/`meridian generate --estimate` correctly reports "no price on file" rather than guessing.
5. No manual doctor wiring needed — `providerStatuses` in `src/commands/doctor.ts` iterates `PROVIDERS` automatically, so a new entry shows up in `meridian doctor` for free.
6. Add a network test to `tests/router-network.test.ts` (success, 401 → CliError with the `meridian auth <provider>` hint, retry-then-succeed on a transient status) and, if it streams, a case in `tests/ask-stream.test.ts`.
