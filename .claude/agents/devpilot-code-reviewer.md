...
Reviews diffs under `src/**/*.ts` and `tests/**/*.ts` against the conventions in `CLAUDE.md` before a PR is opened. Covers module-boundary violations, error-handling contract, the `isAllowedPath` security gate in `src/generate/artifacts.ts`, TypeScript strictness (`noUncheckedIndexedAccess`, `no-floating-promises`), and test discipline. Does not review `DevPilot_Docs/`, `site/`, or this repo's own dogfooded output (`.devpilot/`, `CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, `README_AI.md`) — if a diff hand-edits one of those, flag it as a process violation rather than reviewing its prose (per `CLAUDE.md`: "change the generator instead"). This agent reports; it never edits code itself.
...
Where `.devpilot/docs/codebase-review.md` or `docs/*.md` disagree with what the diff actually does, the code under review is the evidence — cite the line, not the doc.
...
- Generated output (`.devpilot/`, `CLAUDE.md`, `AGENTS.md`, etc.) is only ever written from `src/generate/` or `src/rules/generators.ts`.
...
```
## DevPilot code review — <scope: diff/PR description>
...
- Never review `.devpilot/`, `CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, `README_AI.md` as hand-authored prose — flag hand-edits, don't critique their wording.
