 ...
 * when no provider is available or a call fails — `knowa generate` always
...
export interface GenerateOptions {
  ...
  noAi: boolean;
  /**
   * Files that may be overwritten even without --force: `knowa sync`
 ...
 * dependencies — `knowa generate commands` must not stall waiting for a
 ...
 * construction. A prefix that names an exact file (`.knowa/rules.md`) is
...
const REVIEW_PROMPT = `You are Knowa, a tool that makes codebases AI-assistant-ready.
...
function scaffoldFiles(analysis: ProjectAnalysis): { files: ArtifactFile[]; derived: string[] } {
  return {
    files: [
      {
        file: '.knowa/project.json',
        content:
          JSON.stringify(
            { name: analysis.name, knowa: 1, createdWith: 'knowa generate' },
            null,
            2,
          ) + '\n',
      },
      { file: '.knowa/context.md', content: renderContextMarkdown(analysis) },
      {
        file: 'README_AI.md',
...
/**
 * What a `knowa generate` run would cost, without making a single AI call.
...
export async function runGenerate(opts: GenerateOptions): Promise<GenerateResult> {
  ...
  // manifest so `knowa sync` can later tell user-edited files from
    ...
    try {
      ...
      if (review) {
        ...
        result.files.push(
          ...write(
            'scan',
            [{ file: '.knowa/docs/codebase-review.md', content: review + '\n' }],
            'ai',
            null,
            ['.knowa/docs/codebase-review.md'],
          ),
        );
      }
      ...
      spin.succeed(
        cached
          ...
          : `AI read the codebase${asked} → .knowa/docs/codebase-review.md`,
      );
    } catch (err) {
  ...
  // Record what this run wrote, so `knowa sync` can detect both codebase
