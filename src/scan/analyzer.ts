...
export function renderTree(root: string): string {
  ...
  const level = (dir: string, prefix: string, depth: number): void => {
    ...
    entries.forEach((entry, i) => {
      ...
      if (entry.isDirectory()) level(path.join(dir, entry.name), prefix + (last ? '    ' : '│   '), depth + 1);
    });
  };
  level(root, '', 1);
  return lines.join('\n');
...
> Generated by \`devpilot scan\`. Give this file to AI coding assistants for instant project context.
