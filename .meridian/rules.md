...
- e2e tests: `tests/e2e/*.test.ts`, run with `npm run test:e2e` against vitest.e2e.config.ts; they spawn the *built* CLI as a subprocess (30s timeout) — its own `pretest:e2e` hook runs `npm run build` first, so never assume `dist/` is current otherwise.
