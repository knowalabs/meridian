# Write Tests in Knowa's Vitest Style
...
- Filesystem/vault sandboxing: set `process.env.KNOWA_HOME` to a fresh `fs.mkdtempSync(path.join(os.tmpdir(), 'knowa-<area>-'))` and `process.env.KNOWA_VAULT = 'file'` in `beforeEach`; clean up with `fs.rmSync(dir, { recursive: true, force: true })` in `afterEach`. Never touch the real OS keychain or the developer's real `~/.knowa`.
...
2. Set up the sandbox in `beforeEach`/`afterEach` exactly as described above — temp `KNOWA_HOME`, `KNOWA_VAULT=file`, cleanup — before writing any assertion.
