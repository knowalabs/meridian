# Write Tests for Knowa in This Project's Vitest Style

Write tests for `<module/function under test>` in `@sonalsithara/knowa`, following the exact conventions already established in `tests/*.test.ts` — do not introduce a different testing pattern or library.
...
- Set `process.env.KNOWA_HOME` to a `fs.mkdtempSync(path.join(os.tmpdir(), 'knowa-<area>-'))` temp dir in `beforeEach`, and `process.env.KNOWA_VAULT = 'file'` when vault/config state is involved — never touch the real OS keychain or a developer's real `~/.knowa`.
- Clean up in `afterEach` with `fs.rmSync(dir, { recursive: true, force: true })` and `delete process.env.KNOWA_HOME` (and any other env vars you set).
- For `knowa generate` pipeline tests, build a throwaway target project directory (see `makeProject()` in `tests/generate.test.ts`: writes a minimal `package.json`, `src/index.ts`, `README.md`) rather than pointing tests at this repo itself.
...
- For `CliError` cases, assert both `.message` (what happened) and `.hint` (the actionable next step), matching the pattern in `tests/router-network.test.ts` (e.g. `expect((err as CliError).hint).toContain('knowa auth anthropic')`).
