...
Wiring a new hosted-API or keyless-CLI AI provider into `knowa generate`/`knowa ask`'s router.
...
- Every provider's model is resolved through `modelFor(spec)`: `--model` (`runtimeModels` map via `setRuntimeModel`) → `router.models.<id>` in `~/.knowa/config.json` → `spec.model` default. CLI-backed providers can use the `CLI_DEFAULT_MODEL` sentinel instead of a fixed model name when the signed-in CLI's own config should win.
- Pricing (for `knowa generate --estimate`) is keyed by **model**, not provider, in `MODEL_PRICING` (`pricingFor`) — a model with no entry there and no `router.pricing.<model>` override in the user's config reports no cost rather than a wrong one; do not guess a price.
...
4. If it needs a key, confirm `needsKey: true` and that `verifyApiKey` will work against it (a free, tokenless request) so `knowa auth`/`knowa doctor --online` can validate it.
