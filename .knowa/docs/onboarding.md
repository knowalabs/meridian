...
| Change how `.knowa/rules.md` propagates into tool configs    | `src/rules/generators.ts` — never hand-edit `CLAUDE.md`/`AGENTS.md`/`GEMINI.md` directly and expect it to stick                                                                                                                |
...
| Change the interactive menu (bare `knowa`)                   | `src/launcher.ts` — not unit-tested by design, verify manually                                                                                                                                                                 |
...
- Sandbox filesystem/vault state via `process.env.KNOWA_HOME` (temp dir) and `process.env.KNOWA_VAULT = 'file'`; clean up in `afterEach` with `fs.rmSync(..., { recursive: true, force: true })` — never touch the real OS keychain or the developer's real `~/.knowa`.
...
- **Neither macOS Keychain nor Linux Secret Service can enumerate stored items** — `src/core/vault.ts` keeps a parallel, non-secret `keys/index.json` index under `knowaHome()`. Manipulating the OS vault outside Knowa's `set`/`delete` desyncs this index.
...
- **This repo's own `.knowa/`, `CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, `README_AI.md` are dogfooded `knowa generate` output**, not hand-authored source — don't hand-edit them; change the generator (`src/generate/`, `src/rules/generators.ts`) instead.
